...
function printFailureSummary (failureLines) {
  ...
  const maxLen = 20
  if (hsh.expected && hsh.expected.length > maxLen) { hsh.abbreviated = true }
  if (hsh.actual && hsh.actual.length > maxLen) { hsh.abbreviated = true }
  // console.log(hsh)
  ...
  const msg = `${failureLines[0]} (${append.join(' ')})`
  console.log(msg)
...
rl.on('line', function (line) {
  // State transition.
  if (/^not ok/.test(line)) { currState = 'not-ok' }
  if (line === '  ...' && currState === 'not-ok') { currState = 'ok' }
